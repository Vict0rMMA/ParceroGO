...
function openAddOptionsModal(productId, name, price) {
    ...
    var qtyEl = document.getElementById('add-options-quantity');
...
function animateFlyToCart(count) {
    count = count || 1;
    ...
    fly.textContent = count > 1 ? '+' + count : '+1';
...
function confirmAddWithOptions() {
    ...
    if (qtyEl && qtyEl.value) {
        ...
        if (n >= 1 && n <= 10) quantity = n;
    }
    for (var i = 0; i < quantity; i++) {
        addProduct(addOptionsPending.productId, addOptionsPending.name, addOptionsPending.price, '');
    }
    animateFlyToCart(quantity);
