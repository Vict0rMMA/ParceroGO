"""
Notificaciones por SMS cuando se crea un pedido.
- Siempre: escribe en log (data/notifications.log) y consola.
- Opcional: envía SMS real con Twilio si se configuran las variables de entorno.
"""
...
def _normalize_phone(phone: str) -> str:
    """Formatea el número con código de país +57 para Colombia."""
    if not phone:
        return ""
...
def _get_config():
    """Lee teléfono del dueño desde data/config.json o variable de entorno DELIVERY_OWNER_PHONE."""
...
def _build_sms_text(order: dict) -> str:
    """Construye el texto del mensaje SMS para el dueño del negocio."""
    lines = [
...
def _log_and_print(message: str, order_id: int):
    """Escribe el mensaje en data/notifications.log y en consola."""
...
def _send_twilio(to_phone: str, body: str) -> bool:
    """Envía SMS con Twilio. Devuelve True si se envió correctamente. Requiere variables de entorno."""
...
def send_new_order_sms(order: dict) -> None:
    ...
    Siempre registra en log y consola. Si hay Twilio configurado, envía SMS al dueño.
    """
